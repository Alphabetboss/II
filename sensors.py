...
class DHTSensor:
    ...
    def read(self) -> dict[str, Any]:
        ...
        # Preferred: CircuitPython driver
        ...
        # Mock fallback keeps the app usable on non-Pi machines.
        return {
            "ok": True,
            "humidity": 50.0,
            "temperature_c": 25.0,
...
class MoistureSensor:
    ...
    def read_percent(self) -> float:
        ...
        if self.mode == "ANALOG" and self._adc is not None:
            ...
            # Dry-ish mock value with slight drift
...
class PressureSensor:
    """A simple pressure abstraction.

    Modes:
      - MOCK: returns a healthy baseline.
      - DIGITAL: a pressure switch wired to a GPIO input; active means pressurized.
      - ANALOG: MCP3008 channel mapped to a PSI range.
      - ESP32: optional remote value if present in the field-controller payload.
    """
...
class SensorSuite:
    ...
    def read(self) -> Telemetry:
        ...
        sources = {
            ...
            "moisture": "esp32_serial" if _use_remote_sensors() else config.MOISTURE_MODE.lower(),
            "pressure": "esp32_serial" if _use_remote_sensors() and config.PRESSURE_MODE == "ESP32" else config.PRESSURE_MODE.lower(),
        }
        return Telemetry(
